...
class Stripe(object):
    ...
    def compute_biodescriptors(self, matrix: SparseMatrix, window: int = 3):
        ...
        if submatrix.size == 0:
            self._five_number = np.full(5, -1.0)
            self._inner_mean = -1.0
            self._inner_std = -1.0
            self._outer_lsum = -1.0
            self._outer_rsum = -1.0
            self._outer_lsize = 0
            self._outer_rsize = 0
            return

        # Compute inner descriptors
        self._five_number, self._inner_mean, self._inner_std = self._compute_inner_descriptors(submatrix)

        # Compute outer descriptors
        self._outer_lsum, self._outer_lsize = self._compute_outer_left_descriptors(submatrix, window)
        self._outer_rsum, self._outer_rsize = self._compute_outer_right_descriptors(submatrix, window)
