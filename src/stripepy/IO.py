...
class ResultFile(object):
    ...
    def get(self, chrom: str, location: str, field: Union[str, None] = None) -> pd.DataFrame:
        ...
        mappings = {
            "pseudodistribution": f"/{chrom}/pseudo-distribution/{location}",
            "all_minimum_points": f"/{chrom}/all_minimum_points/{location}",
            "persistence_of_all_minimum_points": f"/{chrom}/persistence_of_all_minimum_points/{location}",
            "all_maximum_points": f"/{chrom}/all_maximum_points/{location}",
            "persistence_of_all_maximum_points": f"/{chrom}/persistence_of_all_maximum_points/{location}",
            "geo_descriptors": f"/{chrom}/stripes/{location}/geo-descriptors",
            "bio_descriptors": f"/{chrom}/stripes/{location}/bio-descriptors",
        }

        if field is not None and field not in mappings:
            raise KeyError(f"Unknown field \"{field}\". Valid fields are {', '.join(mappings.keys())}")

        path = mappings[field]

        if field not in {"geo_descriptors", "bio_descriptors"}:
            return pd.DataFrame({field: self._h5[path][:]})
