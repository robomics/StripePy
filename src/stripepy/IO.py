...
class ResultFile(object):
    ...
    def _update_index(self, chrom: str, name: str, location: str):
        idx = self._chroms_idx[chrom] + 1
        dset_name = f"/index/{location}/{name}"
        self._h5[dset_name][idx] = self._h5[dset_name].shape[0]
    ...
    def _append_pseudodistribution(self, chrom: str, data: Sequence[float], location: str):
        ...
        self._update_index(chrom, "chrom_offsets_pd", location)
    ...
    def _append_persistence(
        ...
        self._update_index(chrom, f"chrom_offsets_{kind}_points", location)
    ...
    def _append_stripes(self, chrom: str, stripes: Sequence[Stripe], location: str):
        ...
        chrom_idx = self._chroms_idx[chrom]
        offset = self._h5[f"/stripes/{location}/seed"].shape[0]
        self._h5[f"/index/{location}/chrom_offsets_stripes"][chrom_idx + 1] = offset
    ...
    def _finalize(self, format_version: Optional[int] = None):
        ...
        num_chroms = len(self._h5["/chroms/name"])
        for prefix, suffix in itertools.product(("/index/lt", "/index/ut"), self._h5["/index/lt"]):
            dset_name = f"{prefix}/{suffix}"
            data = self._h5[dset_name][:]
            data = np.nan_to_num(data, nan=np.nanmax(data))
            if len(data) != num_chroms + 1:
                raise RuntimeError(dset_name, data, len(data), num_chroms + 1)
            assert len(data) == num_chroms + 1
            self._h5[dset_name][:] = data
