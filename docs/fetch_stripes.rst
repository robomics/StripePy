...
By default, this is set to ``None``, meaning no transformation is applied. However, you can specify ``transpose_to_ut``
to transpose coordinates to the upper triangular part of the contact map,
...
Finally, the ``--verbosity`` option allows you to set the level of detail for the console output.
You can choose from standard logging levels: ``debug``, ``info``, ``warning``, ``error``, and ``critical``, with ``info``
