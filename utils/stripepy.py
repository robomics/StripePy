...
def step_1(I, genomic_belt, resolution, RoI=None, output_folder=None):
    ...
    if RoI is not None:
        ...
        if output_folder is not None:
            ...
            # Plots:
            IO.HiC(I_RoI, RoI['genomic'], plot_in_bp=True, output_folder=output_folder,
                   file_name=f"I_{RoI['genomic'][0]}_{RoI['genomic'][1]}.jpg", compactify=False)
            IO.HiC(Iproc_RoI, RoI["genomic"], plot_in_bp=True, output_folder=output_folder,
                   file_name=f"Iproc_{RoI['genomic'][0]}_{RoI['genomic'][1]}.jpg", compactify=False)
    else:
        I_RoI = None
        Iproc_RoI = None
...
def step_2(L, U, resolution, thresh_pers_type, thresh_pers_value, hf, Iproc_RoI=None, RoI=None, output_folder=None):
    ...
    candidate_stripes["lower"] = [stripe.Stripe(seed=LT_MP, top_pers=LT_pers_of_MP, where="lower_triangular")
                                  for LT_MP, LT_pers_of_MP in zip(LT_MPs, LT_pers_of_MPs)]
    candidate_stripes["upper"] = [stripe.Stripe(seed=UT_MP, top_pers=UT_pers_of_MP, where="upper_triangular")
    ...
    if RoI is not None:
        ...
        ids_LT_MPs_in_RoI = np.where(
            (RoI["matrix"][0] <= np.array(LT_MPs)) &
            (np.array(LT_MPs) <= RoI["matrix"][1]))[0]
        LT_MPs_in_RoI = np.array(LT_MPs)[ids_LT_MPs_in_RoI].tolist()

        # Find sites within the range of interest -- upper-triangular:
        ids_UT_MPs_in_RoI = np.where(
            (RoI["matrix"][2] <= np.array(UT_MPs)) &
            (np.array(UT_MPs) <= RoI["matrix"][3]))[0]
        ...
        if output_folder is not None:
            ...
            IO.pseudodistrib(LT_pd, RoI["genomic"][0:2], resolution,
                             output_folder=output_folder, file_name=f"LT_pseudo-distrib.jpg")
            IO.pseudodistrib(UT_pd, RoI["genomic"][0:2], resolution,
                             output_folder=output_folder, file_name=f"UT-pseudo-distrib.jpg")

            # Plot pseudo-distributions and persistent maxima:
            IO.pseudodistrib(LT_pd, RoI["genomic"][0:2], resolution, coords2scatter=[LT_MPs_in_RoI],
                             colors=['blue'], output_folder=output_folder, title=None,
                             file_name=f"LT_pseudo-distrib_and_pers-maxima.jpg", display=False)
            IO.pseudodistrib(UT_pd, RoI["genomic"][0:2], resolution, coords2scatter=[UT_MPs_in_RoI],
                             colors=['blue'], output_folder=output_folder, title=None,
                             file_name=f"UT_pseudo-distrib_and_pers-maxima.jpg", display=False)

            # Plot the region of interest of Iproc with over-imposed vertical lines for seeds:
            if Iproc_RoI is not None:
                IO.HiC_and_sites(Iproc_RoI, LT_MPs_in_RoI, RoI["genomic"], resolution, where="lower",
                                 plot_in_bp=True, output_folder=output_folder, display=False,
                                 file_name=f"LT_seeds.jpg", title=None)
                IO.HiC_and_sites(Iproc_RoI, UT_MPs_in_RoI, RoI["genomic"], resolution, where="upper",
                                 plot_in_bp=True, output_folder=output_folder, display=False,
                                 file_name=f"UT_seeds.jpg", title=None)

    return pseudo_distributions, candidate_stripes


def step_3(L, U, resolution, genomic_belt, max_width, constrain_height, loc_pers_min, loc_trend_min,
           pseudo_distributions, candidate_stripes, hf, Iproc_RoI=None, RoI=None, output_folder=None):
    ...
    if all([param is not None for param in [RoI, output_folder]]):
        ...
        IoIs = ([RoI["genomic"][0:2]] + [[LT_L_bound_in_RoI * resolution, (LT_R_bound_in_RoI + 1) * resolution]
                                         for (LT_L_bound_in_RoI, LT_R_bound_in_RoI) in
                                         zip(LT_L_bounds_in_RoI, LT_R_bounds_in_RoI)])
        IO.pseudodistrib_and_HIoIs(LT_pseudo_distrib, IoIs, resolution,
                                   colors=['red'] + ['blue'] * len(LT_L_bounds_in_RoI),
                                   title=None, output_folder=output_folder,
                                   file_name=f"LT_pseudo-distrib_and_h-doms.jpg", display=False)
        IoIs = ([RoI["genomic"][0:2]] + [[UT_L_bound_in_RoI * resolution, (UT_R_bound_in_RoI + 1) * resolution]
                                         for (UT_L_bound_in_RoI, UT_R_bound_in_RoI) in
                                         zip(UT_L_bounds_in_RoI, UT_R_bounds_in_RoI)])
        IO.pseudodistrib_and_HIoIs(UT_pseudo_distrib, IoIs, resolution,
                                   colors=['red'] + ['blue'] * len(UT_L_bounds_in_RoI),
                                   title=None, output_folder=output_folder,
                                   file_name=f"UT_pseudo-distrib_and_h-doms.jpg", display=False)
        ...
        if Iproc_RoI is not None:
            ...
            LT_bounds_in_RoI_proj_gen_coord = [
                [a[0] * resolution, a[1] * resolution] for a in LT_bounds_in_RoI_proj]
            UT_bounds_in_RoI_proj_gen_coord = [
                [a[0] * resolution, a[1] * resolution] for a in UT_bounds_in_RoI_proj]

            # Slices, i.e., intervals determined by a pair of left & right boundaries:
            LT_slices2keep_proj = [list(range(LT_bound_in_RoI_proj[0], LT_bound_in_RoI_proj[1] + 1))
                                   for LT_bound_in_RoI_proj in LT_bounds_in_RoI_proj]
            UT_slices2keep_proj = [list(range(UT_bound_in_RoI_proj[0], UT_bound_in_RoI_proj[1] + 1))
            ...
            IO.HiC_and_HIoIs(Iproc0_RoI_LT_sliced, LT_bounds_in_RoI_proj_gen_coord, RoI["genomic"], resolution,
                             title=None, output_folder=output_folder, plot_in_bp=True,
                             where="lower", file_name=f"LT_all_h-doms.jpg", display=False)
            ...
            IO.HiC_and_HIoIs(Iproc0_RoI_UT_sliced, UT_bounds_in_RoI_proj_gen_coord, RoI["genomic"], resolution,
                             title=None, output_folder=output_folder, plot_in_bp=True,
                             where="upper", file_name=f"UT_all_h-doms.jpg", display=False)
    ...
    if be_verbose and all([param is not None for param in [RoI, output_folder]]):
        LT_VIoIs, LT_peaks_ids = finders.find_VIoIs(L, LT_MPs, LT_HIoIs,
                                                    VIoIs2plot=ids_LT_MPs_in_RoI,
                                                    max_height=int(genomic_belt / resolution),
                                                    threshold_cut=loc_trend_min,
                                                    min_persistence=loc_pers_min, where="lower",
                                                    output_folder=f"{output_folder}local_pseudodistributions/")
        UT_VIoIs, UT_peaks_ids = finders.find_VIoIs(U, UT_MPs, UT_HIoIs,
                                                    VIoIs2plot=ids_UT_MPs_in_RoI,
                                                    max_height=int(genomic_belt / resolution),
                                                    threshold_cut=loc_trend_min,
                                                    min_persistence=loc_pers_min, where="upper",
                                                    output_folder=f"{output_folder}local_pseudodistributions/")
    else:
        LT_VIoIs, LT_peaks_ids = finders.find_VIoIs(L, LT_MPs, LT_HIoIs,
                                                    VIoIs2plot=None,
    ...
    if RoI is not None:
        ...
        LT_HIoIs_in_RoI_proj = [[
            LT_HIoI_RoI[0] - RoI["matrix"][0],
            LT_HIoI_RoI[1] - RoI["matrix"][0]] for LT_HIoI_RoI in LT_HIoIs_in_RoI]
        UT_HIoIs_in_RoI_proj = [[
            UT_HIoI_RoI[0] - RoI["matrix"][0],
            UT_HIoI_RoI[1] - RoI["matrix"][0]] for UT_HIoI_RoI in UT_HIoIs_in_RoI]
        LT_VIoIs_in_RoI_proj = [[
            LT_VIoI_RoI[0] - RoI["matrix"][0],
            LT_VIoI_RoI[1] - RoI["matrix"][0]] for LT_VIoI_RoI in LT_VIoIs_in_RoI]
        UT_VIoIs_in_RoI_proj = [[
            UT_VIoI_RoI[0] - RoI["matrix"][0],
            UT_VIoI_RoI[1] - RoI["matrix"][0]] for UT_VIoI_RoI in UT_VIoIs_in_RoI]

        if constrain_height:
            LT_peaks_ids_RoI = [LT_peaks_ids_in_candida_in_RoI
                                for n, LT_peaks_ids_in_candida_in_RoI in enumerate(LT_peaks_ids) if
                                n in ids_LT_MPs_in_RoI]
            LT_peaks_ids_RoI_proj = [[LT_peak_idx_in_candida_in_RoI - RoI["matrix"][0]
            ...
            UT_peaks_ids_RoI = [UT_peaks_ids_in_candida_in_RoI
                                for n, UT_peaks_ids_in_candida_in_RoI in enumerate(UT_peaks_ids) if
                                n in ids_UT_MPs_in_RoI]
            UT_peaks_ids_RoI_proj = [[UT_peak_idx_in_candida_in_RoI - RoI["matrix"][0]
        ...
        IO.plot_stripes(Iproc_RoI, LT_HIoIs_in_RoI_proj, LT_VIoIs_in_RoI_proj, [], [], RoI["genomic"],
                        resolution, plot_in_bp=True,
                        output_folder=output_folder,
                        file_name=f"LT_all_candidates.jpg", title=None,
                        display=False)

        if constrain_height:
            IO.plot_stripes_and_peaks(Iproc_RoI, LT_HIoIs_in_RoI_proj, LT_VIoIs_in_RoI_proj, [], [],
                                      LT_peaks_ids_RoI_proj, [], RoI["genomic"],
                                      resolution, plot_in_bp=True,
                                      output_folder=output_folder,
                                      file_name=f"LT_all_candidates_and_peaks.jpg", title=None,
                                      display=False)

        IO.plot_stripes(Iproc_RoI, [], [], UT_HIoIs_in_RoI_proj, UT_VIoIs_in_RoI_proj, RoI["genomic"],
                        resolution, plot_in_bp=True,
                        output_folder=output_folder,
                        file_name=f"UT_all_candidates.jpg", title=None,
                        display=False)

        if constrain_height:
            IO.plot_stripes_and_peaks(Iproc_RoI, [], [], UT_HIoIs_in_RoI_proj, UT_VIoIs_in_RoI_proj,
                                      [], UT_peaks_ids_RoI_proj, RoI["genomic"],
                                      resolution, plot_in_bp=True,
                                      output_folder=output_folder,
                                      file_name=f"UT_all_candidates_and_peaks.jpg", title=None,
                                      display=False)
    ...
    if be_verbose and output_folder is not None:
        ...
        sns.histplot(data=pd.DataFrame(LT_widths), kde=False, legend=False, fill=True, discrete=True, color="#2F539B",
        ...
        plt.clf()
        fig, ax = plt.subplots(1, 1)
        sns.histplot(data=pd.DataFrame(UT_widths), kde=False, legend=False, fill=True, discrete=True, color="#2F539B",
                     edgecolor=None, alpha=1)
        plt.xlim(0, max(max(LT_widths), max(UT_widths)) + 1)
        plt.title("Widths")
        ...
        plt.clf()
        fig, ax = plt.subplots(1, 1)
        sns.histplot(data=pd.DataFrame(LT_heights), kde=False, legend=False, fill=True, discrete=True, color="#2F539B",
                     edgecolor=None, alpha=1)
        ...
        plt.clf()
        fig, ax = plt.subplots(1, 1)
        sns.histplot(data=pd.DataFrame(UT_heights), kde=False, legend=False, fill=True, discrete=True, color="#2F539B",
                     edgecolor=None, alpha=1)
...
def step_4(L, U, candidate_stripes, hf, resolution=None, thresholds_relative_change=None, Iproc_RoI=None, RoI=None,
           output_folder=None):
    ...
    if all(param is not None for param in [resolution, thresholds_relative_change, Iproc_RoI, RoI, output_folder]):
        ...
        for threshold in thresholds_relative_change:
            ...
            # Filtration:
            LT_candidates2keep = [index for index, rel_change in enumerate(LT_biological_descriptors['relative change'])
                                  if rel_change >= threshold]
            UT_candidates2keep = [index for index, rel_change in enumerate(UT_biological_descriptors['relative change'])
                                  if rel_change >= threshold]
            ...
            if RoI is not None:
                ...
                LT_filt_HIoIs_in_RoI_proj = [[a[0] - RoI["matrix"][0], a[1] - RoI["matrix"][2]] for a in LT_filt_HIoIs_in_RoI]
                LT_filt_VIoIs_in_RoI = np.array(LT_filt_VIoIs)[LT_candidates2keep_in_RoI].tolist()
                LT_filt_VIoIs_in_RoI_proj = [[a[0] - RoI["matrix"][0], a[1] - RoI["matrix"][2]] for a in LT_filt_VIoIs_in_RoI]

                IO.plot_stripes(Iproc_RoI, LT_filt_HIoIs_in_RoI_proj, LT_filt_VIoIs_in_RoI_proj, [], [],
                                RoI["genomic"], resolution, plot_in_bp=True,
                                output_folder=f"{output_folder}",
                ...
                UT_filt_HIoIs_in_RoI_proj = [[a[0] - RoI["matrix"][0], a[1] - RoI["matrix"][2]] for a in UT_filt_HIoIs_in_RoI]
                UT_filt_VIoIs_in_RoI = np.array(UT_filt_VIoIs)[UT_candidates2keep_in_RoI].tolist()
                UT_filt_VIoIs_in_RoI_proj = [[a[0] - RoI["matrix"][0], a[1] - RoI["matrix"][2]] for a in UT_filt_VIoIs_in_RoI]

                IO.plot_stripes(Iproc_RoI, [], [], UT_filt_HIoIs_in_RoI_proj, UT_filt_VIoIs_in_RoI_proj,
                                RoI["genomic"], resolution, plot_in_bp=True,
                                output_folder=f"{output_folder}",
