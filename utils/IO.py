...
def list_folders_for_plots(path):
    ...
    folders4plots += [folders4plots[0] + "/1_preprocessing",
...
def pseudodistrib(pseudo_distrib, IoI, resolution, coords2scatter=None, colors=None, output_folder=None,
    ...
    fig, ax = plt.subplots(1, 1)
    ax.plot(range(IoI[0], IoI[1], resolution), pseudo_distrib[int(IoI[0]/resolution):int(IoI[1]/resolution)],
    ...
    if coords2scatter is not None:
        for n, cur_coords2scatter in enumerate(coords2scatter):
            if colors is not None:
                ...
                color = 'red'
            ax.plot([cur_coord2scatter*resolution for cur_coord2scatter in cur_coords2scatter],
                    pseudo_distrib[cur_coords2scatter],
                    marker='.', linestyle='', markersize=6 * 1.5, color=color)
    ax.xaxis.set_major_formatter(EngFormatter('b'))
...
def pseudodistrib_and_HIoIs(pseudo_distrib, IoIs, resolution, colors=None, output_folder=None,
                            file_name=None, title=None, display=False):
    """
    :param pseudo_distrib:          1D ndarray representing a scalar function sampled over a uniform mesh
...
def HiC_and_sites(I, sites, RoI, resolution, where=None, plot_in_bp=False, output_folder=None, file_name=None,
    ...
    for site in [site*resolution for site in sites]:
        if where == "lower":
            ax.plot([site, site],
                    [site, RoI[1] - 1*resolution],
                    color=(0.0, 0.0, 1.0), linestyle='dashed', linewidth=1)
        elif where == "upper":
            ax.plot([site, site],
                    [RoI[0] + 1*resolution, site],
                    color=(0.0, 0.0, 1.0), linestyle='dashed', linewidth=1)
        else:
            ax.plot([site, site],
...
def HiC_and_HIoIs(I, HIoIs, RoI, resolution, where=None, plot_in_bp=False, output_folder=None, file_name=None,
                  title=None, display=False):
    """
    :param I:                   Hi-C matrix to be plotted as image and saved
    ...
    for HIoI in HIoIs:
        if where == "lower":
            ax.plot([RoI[0] + HIoI[0], RoI[0] + HIoI[0]],
                    [RoI[2] + HIoI[0], RoI[2] + (I.shape[0] - 1)*resolution],
                    color=(0.0, 0.0, 1.0), linestyle='dashed', linewidth=1)
            ax.plot([RoI[0] + HIoI[1] + resolution, RoI[0] + HIoI[1] + resolution],
...
def plot_stripes(I, LT_HIoIs, LT_VIoIs, UT_HIoIs, UT_VIoIs, RoI, resolution,
                 plot_in_bp=False, output_folder=None, file_name=None,
                 title=None, display=False):
    """
     :param I:                  Hi-C matrix to be plotted as image and saved
    ...
    for num_window, (HIoI, VIoI) in enumerate(zip(UT_HIoIs, UT_VIoIs)):
        ...
        if low_idx_up_opt != low_idx_dw_opt and low_idx_sx_opt != low_idx_dx_opt:
            ax.plot(
                ...
                'blue', linestyle='dashed', linewidth=1)
            ax.plot(
...
def plot_stripes_and_peaks(I, LT_HIoIs, LT_VIoIs, UT_HIoIs, UT_VIoIs, LT_peaks_ids, UT_peaks_ids, RoI, resolution,
                           plot_in_bp=False, output_folder=None, file_name=None,
                           title=None, display=False):
