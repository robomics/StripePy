...
class UnionFind:
    ...
    It keeps track of elements partitioned into a number of disjoint (non-overlapping) sets.
    This data structure is required for merge trees and similar algorithms.

    This implementation uses path compression in several places, written with a merge-tree-like algorithm in mind.
    ...
    def FindAndCompress(self, idxElement):
        ...
        # ~ Compress the path
        for idxStep in Path:
            self.Domain[idxStep] = idxElement
